...
const Input = ({ className, type, ...props }: ComponentPropsWithRef<"input">) => {
    return (
        <input
            type={type}
            className={cn(
                "flex h-8 w-full rounded-md border border-main/60 bg-primary px-3 py-1 shadow-sm dark:shadow-md transition-colors file:border-0 text-secondary focus:outline-0" +
