...
const CircleFilled = ({className, size = 16, ...props}: SpinnerVariantProps) => (
    <div className="relative" style={{ width: size, height: size }}>
        <div className="absolute inset-0 rotate-180">
            <LoaderCircleIcon
                className={cn('animate-spin', className, 'text-secondary opacity-20')}
                size={size}
                {...props}
            />
        </div>
...
const Infinite = ({ size = 16, ...props }: SpinnerVariantProps) => (
    <svg
        height={size}
...
export const Spinner = ({ variant, ...props }: SpinnerProps) => {
    switch (variant) {
        ...
        default:
            return <Default className={"mr-2"} size={16} {...props} />
    }
}